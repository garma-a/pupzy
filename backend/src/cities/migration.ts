...
import {
  ...
  generateCityCatalogRevisionSql,
} from './release-sql';
...
export function generateReleaseMigrationSql(
  ...
  const lines: string[] = [
    ...
    'BEGIN',
    '  -- 1. Acquire the City cache fence before changing catalog data',
    ...generateCityCatalogRevisionSql('  '),
    '',
    '  -- 2. Apply reviewed City identity transfers onto existing application UUIDs',
    ...generateCityIdentityTransfersSql(release.identityTransfers ?? [], '  '),
    '',
    '  -- 3. Mark newly retired cities as RETIRED',
    ...generateRetiredCitiesSql(updatedCatalog, '  '),
    '',
    '  -- 4. Upsert official and updated catalog records',
    ...generateCitiesUpsertSql(updatedCatalog, '  '),
    '',
    '  -- 5. Synchronize denormalized governorate values in posts for official cities',
    generatePostGovernorateSyncSql('  '),
    '',
    `  -- 6. Verification checks: assert exactly ${officialCount} official cities and ${expectedGovCount} governorates`,
