/* eslint-disable @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unsafe-call */
...
function claimedPostTicket(): StagedUpload {
  return {
    id: 'media-1',
    userId: 'user-1',
    purpose: 'POST_MEDIA',
...
function ticketSelectResult(ticket: StagedUpload) {
  return {
...
function createFinalizationDbMock({
  ...
  return {
    ...
    select: jest.fn().mockImplementation(() => {
      topLevelSelectCount++;
      if (topLevelSelectCount === 1) {
        return ticketSelectResult(claimedPostTicket());
      }
      if (topLevelSelectCount % 2 === 0) {
        return {
          from: jest.fn().mockReturnValue({
...
describe('UploadService', () => {
  ...
  beforeEach(() => {
    ...
    mockDb.transaction = jest.fn().mockImplementation(async (cb: (tx: any) => Promise<unknown>) => {
      let selectCount = 0;
      const tx = {
  ...
  describe('generatePresignedUrl', () => {
    ...
    it('binds persisted upload grace deadline and getSignedUrl signature to the exact same timestamp', async () => {
      ...
      expect(mockCache.set).toHaveBeenCalledWith(`media_ct:${result.mediaId}`, 'image/jpeg', 900_000);
      expect(mockCache.set).toHaveBeenCalledWith(`media_owner:${result.mediaId}`, 'user-1', 900_000);
    });
  });
  ...
  describe('getPublicCdnUrl & purgeCdn & getPurgeCdnUrls', () => {
    ...
    it('aborts finalization and cleans up staging when user is deleted or deletion is pending', async () => {
      ...
      const mockDb = {
        ...
        select: jest.fn().mockReturnValue(ticketSelectResult(claimedPostTicket())),
      };
    ...
    it('compensates by deleting the permanent object when acceptance commits during the copy', async () => {
      ...
      const mockDb = {
        ...
        select: jest
          .fn()
          .mockReturnValueOnce(ticketSelectResult(claimedPostTicket()))
          .mockReturnValue({
            from: jest.fn().mockReturnValue({
