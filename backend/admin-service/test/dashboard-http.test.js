...
before(async () => {
  ...
  superCookie = (await login("admin@example.com", "super secure password")).cookie;
  staffCookie = (await login("staff@example.com", "staff secure password")).cookie;
});
...
describe("AdminJS Redis-free dashboard and cache invalidation HTTP behavior", () => {
  ...
  it("successful banUser invalidates cache immediately and updates dashboard statistics", async () => {
    ...
    const actionRes = await postAction(
      `/admin/api/resources/users/records/${targetUserId}/banUser`,
  ...
  it("successful unbanUser invalidates cache immediately and updates dashboard statistics", async () => {
    ...
    const userRes = await database.pool.query(
      `SELECT id FROM users WHERE is_banned = true LIMIT 1`,
    );
    const targetUserId = userRes.rows[0].id;
    ...
    const actionRes = await postAction(
      `/admin/api/resources/users/records/${targetUserId}/unbanUser`,
      {},
    );
    assert.equal(actionRes.status, 200);
    assert.equal(actionRes.data?.notice?.type, "success");
  ...
  it("built-in AdminJS mutations invalidate cache immediately", async () => {
    ...
    const editRes = await postAction(
  ...
  it("failed, rejected, and unauthorized mutations do not invalidate cache", async () => {
    ...
    mockTime += 1_000;
    const failRes = await postAction(
      `/admin/api/resources/users/records/${principals.userId}/banUser`,
      { reason: "   " },
    );
  ...
  it("read-only actions do not invalidate cache", async () => {
    ...
    assert.equal(
      afterReads.stats.computedAt,
      initialTimestamp,
      "read-only actions must not invalidate cache",
    );
  });
  ...
  it("deterministic mutation/computation race test verifies pre-mutation computation does not repopulate cache", async () => {
    ...
    // Cache must remain null after invalidation
    assert.equal(raceBuilt.cache.cached, null, "stale computation must not overwrite invalidated cache");
