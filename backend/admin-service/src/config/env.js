...
const envSchema = z.object({
  ...
  ADMIN_COOKIE_PASSWORD: z.string().min(32, 'ADMIN_COOKIE_PASSWORD must be at least 32 characters'),
  ...
  NOMINATIM_ATTRIBUTION: z
    .string()
    .default('Data &copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors, ODbL 1.0'),
...
export function validateEnv(raw) {
  ...
  if (!result.success) {
    const lines = result.error.issues.map((issue) => `  ${issue.path.join('.')}: ${issue.message}`).join('\n');
