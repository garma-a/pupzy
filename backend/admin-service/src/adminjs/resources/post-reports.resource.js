...
export function buildPostReportsResource(db, components = {}, pool) {
  const properties = {
  ...
  return buildReadOnlyResource(db, 'post_reports', { name: 'Moderation', icon: 'Flag' }, properties, {
    ...
    filterProperties: ['reason', 'post_id', 'reporter_id', 'reviewed_at', 'review_outcome', 'created_at'],
    actions: {
      list: { after: stripPopulatedPasswordHashes },
      show: { after: stripPopulatedPasswordHashes },
      reviewWithNoAction: buildPostReportReviewAction(pool, components?.ModerationAction),
    },
  });
}
