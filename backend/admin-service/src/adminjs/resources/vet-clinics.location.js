 ...
 * (discrete lat/lng fields, EWKT POINT(lng lat), lat,lng strings, or JSON).
 */
export function parseCoordinates(payload) {
  ...
  if (typeof payload.coordinates === "object" && payload.coordinates !== null) {
    const lat = Number(
      payload.coordinates.latitude ?? payload.coordinates.lat,
    );
    const lng = Number(
      payload.coordinates.longitude ?? payload.coordinates.lng,
    );
    return { lat, lng };
  }
  ...
  if (typeof payload.coordinates === "string") {
    const raw = payload.coordinates.trim();
...
export function buildGoogleMapsUrl(latitude, longitude) {
  ...
  if (
    ...
    throw new Error(
      `Invalid WGS84 coordinates: latitude=${latitude}, longitude=${longitude}`,
    );
  }
...
/**
 * Determines whether location-specific properties were included in an edit payload.
 */
export function isLocationModified(payload, existingRecord) {
  ...
  if (
    ...
    existingRecord &&
    payload.city_id !== existingRecord.city_id
  ) {
    return true;
  }
...
export function validateMappedLocation(payload, options = {}) {
  ...
  const addressEnglish =
    typeof payload.address_english === "string"
      ? payload.address_english.trim()
      : "";
  if (!addressEnglish) {
    throw new ValidationError({
  ...
  const addressArabic =
    typeof payload.address_arabic === "string"
      ? payload.address_arabic.trim()
      : "";
  if (!addressArabic) {
    throw new ValidationError({
