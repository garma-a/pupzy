...
describe("Curated Detail, Engagement, Reference, and Administration Tables (Task 11)", () => {
  ...
  describe("Reference Resources Curation", () => {
    ...
    it("vet_clinics exposes bilingual addresses and omits raw coordinates from list", () => {
  ...
  describe("Admin & Audit Resources Curation", () => {
    ...
    it("vet_clinic_location_audits is read-only, hides raw coordinates from list, and uses ShortUuid", () => {
  ...
  describe("Global Table Quality Across ALL 20 Registered Resources", () => {
    it("every registered resource in ADMIN_RESOURCE_TABLES defines explicit listProperties and no unsafe defaults", () => {
      ...
      for (const table of Object.keys(resourcesByTable)) {
        ...
        assert.ok(
          Array.isArray(res.options.listProperties),
          `${table} must have explicit listProperties array`,
        );
        assert.ok(
        ...
        assert.ok(
          Array.isArray(res.options.showProperties),
          `${table} must have explicit showProperties array`,
        );
        assert.ok(
          Array.isArray(res.options.filterProperties),
          `${table} must have explicit filterProperties array`,
        );
        assert.equal(
          res.options.actions.delete?.isAccessible,
          false,
          `${table} must disable hard delete`,
        );
        assert.equal(
          res.options.actions.bulkDelete?.isAccessible,
          false,
          `${table} must disable bulk delete`,
        );
      }
    });
  });
