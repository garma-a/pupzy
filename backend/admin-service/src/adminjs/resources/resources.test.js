...
function values(resource, property) {
  return resource.options.properties[property].availableValues.map(({ value }) => value);
}
...
describe('AdminJS resource configuration', () => {
  ...
  it('transcribes every enum-backed property exactly', () => {
    const byTable = Object.fromEntries(resources().map((item) => [item.resource.name, item]));
    const expectations = [
  ...
  it('disables hard delete and bulk delete on every resource', () => {
    for (const resource of resources()) {
      assert.equal(resource.options.actions.delete.isAccessible, false, `${resource.resource.name}.delete`);
      assert.equal(resource.options.actions.bulkDelete.isAccessible, false, `${resource.resource.name}.bulkDelete`);
    }
  });
  ...
  it('enforces explicit listProperties on every registered resource to prevent automatic column fallback', () => {
    for (const resource of resources()) {
      ...
      // Ensure no coordinates or password hashes in list properties
      assert.ok(
        !listProps.includes('coordinates'),
        `${resource.resource.name} must not expose coordinates in listProperties`,
      );
      assert.ok(
  ...
  it('attaches cache invalidation hooks to enabled mutating actions and preserves existing after hooks', async () => {
    ...
    const wrapped = (await import('./resource-helpers.js')).attachCacheInvalidation(testResource, cache);
