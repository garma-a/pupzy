...
describe('AdminJS Adoption Applications Resource Configuration', () => {
  it('declares concise intentional listProperties excluding detailed questionnaires', () => {
    ...
    assert.equal(resource.options.listProperties.includes('previous_pet_experience'), false);
  });
  ...
  it('attaches ShortUuid custom component to ID and relation fields when provided', () => {
    ...
    assert.equal(resource.options.properties.target_post_id.components.list, 'CustomShortUuidComponent');
    assert.equal(resource.options.properties.target_post_id.components.show, 'CustomShortUuidComponent');
    assert.equal(resource.options.properties.applicant_id.components.list, 'CustomShortUuidComponent');
    assert.equal(resource.options.properties.applicant_id.components.show, 'CustomShortUuidComponent');
  });
  ...
  it('strips populated password hashes from list and show after hooks', () => {
    ...
    assert.equal(cleaned.record.populated.applicant_id.params.password_hash, undefined);
  });
  ...
  it('transcribes all enum-backed preference and status fields accurately', () => {
    ...
    assert.deepEqual(
      resource.options.properties.species_preference.availableValues.map((v) => v.value),
      ENUMS.speciesType,
    );
    assert.deepEqual(
      resource.options.properties.gender_preference.availableValues.map((v) => v.value),
      ENUMS.genderType,
    );
    assert.deepEqual(
      resource.options.properties.living_situation.availableValues.map((v) => v.value),
