...
describe("Moderation and Activity Tables Curation & Layout Rules (Task 10)", () => {
  ...
  describe("Pupzy Theme CSS Table & Overflow Rules", () => {
    it("configures table wrappers for explicit horizontal overflow scrolling", () => {
      assert.match(
        ...
        "Table wrappers must enable horizontal scrolling",
      );
      assert.match(
        cssContent,
        /min-width:\s*600px/,
    ...
    it("enforces no-wrap on IDs, timestamps, enums, counts, badges, and action cells", () => {
      assert.match(
        ...
        "IDs must be styled with no-wrap",
      );
      assert.match(
        cssContent,
        /td\[data-property-name\*="_at"\]/,
        "Timestamps must be styled with no-wrap",
      );
      assert.match(
        cssContent,
        /td\[data-property-name\*="status"\]/,
        "Statuses must be styled with no-wrap",
      );
      assert.match(
        cssContent,
        /td\.adminjs_TableActionCell/,
        "Action cell must be styled with no-wrap",
      );
    });
    ...
    it("preserves full text wrapping on record show views", () => {
      ...
      assert.match(
        ...
        "Show views must allow multiline text",
      );
    });
  ...
  describe("Moderation Resources Curation", () => {
    it("posts resource declares concise scan-oriented columns and ShortUuid", () => {
      ...
      assert.ok(
        !postsRes.options.listProperties.includes("description"),
        "description must not be in listProperties",
      );
      assert.ok(
        !postsRes.options.listProperties.includes("coordinates"),
        "coordinates must not be in listProperties",
      );
      assert.equal(
        postsRes.options.properties.id.components.list,
        "ShortUuidMock",
      );
      assert.equal(
        postsRes.options.properties.creator_id.components.show,
        "ShortUuidMock",
      );
    });
    ...
    it("users resource declares concise scan-oriented columns and ShortUuid", () => {
      ...
      assert.equal(
        usersRes.options.properties.id.components.list,
        "ShortUuidMock",
      );
      assert.equal(
        usersRes.options.properties.banned_by_admin_id.components.show,
        "ShortUuidMock",
      );
    });
    ...
    it("post_reports resource exposes moderation context without long details in list", () => {
      ...
      assert.equal(
        reportsRes.options.properties.id.components.list,
        "ShortUuidMock",
      );
      assert.equal(
        reportsRes.options.properties.post_id.components.list,
        "ShortUuidMock",
      );
      assert.equal(
        reportsRes.options.properties.reporter_id.components.list,
        "ShortUuidMock",
      );
    });
  });
  ...
  describe("User Activity Resources Curation", () => {
    it("contact_requests resource uses concise columns and ShortUuid", () => {
      ...
      assert.equal(
        contactRes.options.properties.id.components.list,
        "ShortUuidMock",
      );
      assert.equal(
        contactRes.options.properties.post_id.components.list,
        "ShortUuidMock",
      );
      assert.equal(
        contactRes.options.properties.requester_id.components.list,
        "ShortUuidMock",
      );
    });
    ...
    it("adoption_applications resource uses concise columns and ShortUuid", () => {
      ...
      assert.equal(
        adoptRes.options.properties.id.components.list,
        "ShortUuidMock",
      );
      assert.equal(
        adoptRes.options.properties.target_post_id.components.list,
        "ShortUuidMock",
      );
      assert.equal(
        adoptRes.options.properties.applicant_id.components.list,
        "ShortUuidMock",
      );
    });
    ...
    it("saved_searches resource uses intentional columns, readable City and ShortUuid", () => {
      ...
      assert.equal(
        savedRes.options.properties.id.components.list,
        "ShortUuidMock",
      );
      assert.equal(
        savedRes.options.properties.user_id.components.list,
        "ShortUuidMock",
      );
    });
    ...
    it("notifications resource uses intentional columns and ShortUuid", () => {
      ...
      assert.equal(
        notifRes.options.properties.id.components.list,
        "ShortUuidMock",
      );
      assert.equal(
        notifRes.options.properties.recipient_id.components.list,
        "ShortUuidMock",
      );
    });
  });
  ...
  describe("Security and Privacy Posture across Moderation & Activity Resources", () => {
    ...
    it("all resources disable destructive delete and bulkDelete actions", () => {
      for (const res of resources) {
        assert.equal(
          res.options.actions.delete.isAccessible,
          false,
          `${res.resource.name} must disable delete`,
        );
        assert.equal(
