...
describe("AdminJS Posts Resource Configuration & City/Governorate Contracts", () => {
  ...
  it("wires ShortUuid component to id, creator_id, and moderated_by_admin_id", () => {
    ...
    assert.equal(
      res.options.properties.creator_id.components.show,
      "CustomShortUuid",
    );
    assert.equal(
      res.options.properties.moderated_by_admin_id.components.show,
      "CustomShortUuid",
    );
  });
  ...
  it("strips populated password hashes from list and show after hooks", () => {
    ...
    assert.equal(
      cleaned.record.populated.creator_id.params.password_hash,
      undefined,
    );
  });
  ...
  it("edit before hook rejects non-official (LEGACY / RETIRED / non-existent) city_id", async () => {
    ...
    await assert.rejects(
      ...
      (err) => {
        ...
        assert.equal(
          err.propertyErrors.city_id.message,
          "Must select an existing official City",
        );
        return true;
      },
    );
