...
describe("AdminJS Contact Requests Resource Configuration", () => {
  ...
  it("attaches ShortUuid custom component to ID and relation fields when provided", () => {
    ...
    assert.equal(
      resource.options.properties.post_id.components.list,
      "CustomShortUuidComponent",
    );
    assert.equal(
      resource.options.properties.post_id.components.show,
      "CustomShortUuidComponent",
    );
    assert.equal(
      resource.options.properties.requester_id.components.list,
      "CustomShortUuidComponent",
    );
    assert.equal(
      resource.options.properties.requester_id.components.show,
      "CustomShortUuidComponent",
    );
  });
  ...
  it("strips populated password hashes from list and show after hooks", () => {
    ...
    assert.equal(
      cleaned.record.populated.requester_id.params.password_hash,
      undefined,
    );
  });
  ...
  it("transcribes request status enum values exactly", () => {
    const resource = buildContactRequestsResource(db);
    const available = resource.options.properties.status.availableValues.map(
      (v) => v.value,
    );
