...
describe("AdminJS Post Reports Resource Configuration", () => {
  ...
  it("attaches ShortUuid custom component to ID and relation fields when provided", () => {
    ...
    assert.equal(
      resource.options.properties.reporter_id.components.list,
      "CustomShortUuidComponent",
    );
    assert.equal(
      resource.options.properties.reporter_id.components.show,
      "CustomShortUuidComponent",
    );
  });
  ...
  it("strips populated password hashes from list and show after hooks", () => {
    ...
    assert.equal(
      cleaned.record.populated.reporter_id.params.password_hash,
      undefined,
    );
  });
  ...
  it("transcribes report reason enum values exactly", () => {
    const resource = buildPostReportsResource(db);
    const available = resource.options.properties.reason.availableValues.map(
      (v) => v.value,
    );
