...
- Migration `0054_add_profile_photo_lifecycle` adds the `PROFILE_PHOTO` staged-upload purpose and the `users.profile_photo_storage_key` / `users.profile_photo_changed_at` columns. It is additive and leaves existing provider pictures and Post/Comment media untouched.
