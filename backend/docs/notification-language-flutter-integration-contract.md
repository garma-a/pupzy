...
Every type persisted by the `notification_type` enum has English and Arabic definitions: `NEW_UPVOTE`, `POST_SAVED`, `CONTACT_REQUEST_RECEIVED`, `CONTACT_REQUEST_APPROVED`, `CONTACT_REQUEST_REJECTED`, `ADOPTION_APPLICATION_RECEIVED`, `ADOPTION_APPLICATION_APPROVED`, `ADOPTION_APPLICATION_REJECTED`, `POST_REMOVED_BY_ADMIN` (single-post and account-ban cascade variants), `POST_INACTIVITY_NUDGE`, `SYSTEM_ANNOUNCEMENT`, `NEW_COMMENT`, `NEW_REPLY`, `COMMENT_BOOSTED`, `COMMENT_PINNED`.

Administrative notifications written directly by the AdminJS service (`POST_REMOVED_BY_ADMIN`) now carry both languages. Rows that predate this work keep the English fallback.
