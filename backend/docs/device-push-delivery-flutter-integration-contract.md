...
> **Delivery coverage in this slice:** durable push is enabled for the existing **adoption-approval** notification (`ADOPTION_APPLICATION_APPROVED`). The device registry, preference and delivery machinery are generic; remaining workflow pushes (contacts, discussion, moderation, outcomes, expiry/reminders) are enabled by the follow-up push-coverage work and are not yet sent.
...
- `notification.title` / `notification.body` — localized text (see above).
- `data` — tap-routing metadata; every value is a string. Present keys depend on the notification:

| Key | Meaning |
|---|---|
| `notificationId` | The inbox notification ID (use it to mark read / deep link). |
| `type` | Notification type enum value, e.g. `ADOPTION_APPLICATION_APPROVED`. |
...
- Collapse controls: Android `collapseKey` and APNs `apns-collapse-id` carry the same bounded key.
...
- Enabling push for notification types other than `ADOPTION_APPLICATION_APPROVED` (follow-up push-coverage work; the machinery is already generic).
