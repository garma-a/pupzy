...
**Rollout ordering (operationally binding).** This release ships the retired saved-search runtime and `0053_drop_saved_searches` in the same revision, so `0053` must not be applied by a rolling deploy: a pre-19 API revision (whose Account Deletion cleanup still queries `saved_searches`) or a pre-18 AdminJS revision (which registers the resource) can still be serving when the new revision's pre-deploy drops the table. Apply it stop-the-world — scale the Main API and AdminJS to zero, run `node dist/database/migrate.js` (or the new revision's pre-deploy) with no containers serving, then start the new revisions — or cut a runtime-only intermediate release containing ticket 19's code changes without `0053`, drain it, and only then drop. The Main API is the sole migration owner and AdminJS never runs migrations. All other new migrations (`0045`–`0052`, `0054`, `0055`) are additive and safe to run while the previous code revision is still serving; `0056` rebuilds one index non-concurrently and fits the stopped window. Full ordering is in `docs/integrated-mvp-frontend-handoff.md` §6 and `docs/deployment/three-service-railway-release.md` §4.
