...
**Rollout ordering (operationally binding).** This release ships the retired saved-search runtime and `0053_drop_saved_searches` in the same revision, so `0053` must not be applied by a rolling deploy: a pre-19 API revision (whose Account Deletion cleanup still queries `saved_searches`) or a pre-18 AdminJS revision (which registers the resource) can still be serving when the new revision's pre-deploy drops the table. Apply it stop-the-world — scale the Main API and AdminJS to zero, run `node dist/database/migrate.js` (or the new revision's pre-deploy) with no containers serving, then start the new revisions — or cut a runtime-only intermediate release containing ticket 19's code changes without `0053`, drain it, and only then drop; this release does not provide that intermediate artifact. The Main API is the sole migration owner and AdminJS never runs migrations. All other new migrations (`0045`–`0052`, `0054`, `0055`) are additive and safe to run while the previous code revision is still serving; `0056` rebuilds one index non-concurrently and fits the stopped window. Full ordering is in `docs/integrated-mvp-frontend-handoff.md` §6 and `docs/deployment/three-service-railway-release.md` §4.
...
## 16. Second post-review round (PR review of `3b12b7c`)

Three further confirmed findings from the PR review were fixed on `task/bwg-fix3` and merged (`ef14a94`); all were independently reviewed with red/green reproduction.

| Finding | Fix commits | Resolution and evidence |
|---|---|---|
| P2 — avatar cleanup could delete a newly activated avatar (`media-deletion.processor.ts` checked the reference, then deleted without coordinating with activation) | `a4f74d6`, `eb2998e` | Recovery now reclaims an orphan inside one transaction that locks the owner `users` row (the same lock activation takes), re-checks the reference, marks the ticket terminal (`EXPIRED`) and durably queues final + staging deletion work before any storage I/O. Activation re-verifies the ticket is still `FINALIZED` with the same `finalStorageKey` and rejects a reclaimed ticket with `PROFILE_PHOTO_REPLACED`. Both interleavings are tested; pre-fix, the delayed activation installed bytes already deleted. Hardening `eb2998e` also closes two leak corners: an owner row deleted between scan and reclaim now still queues durable deletion work, and a finalize that loses the `CLAIMED → FINALIZED` transition discards/queues the just-published object and throws a stable retryable error. |
| P1 — the documented rollout dropped `saved_searches` while pre-19 revisions could still query it during Account Deletion | `7fe2ca4`, `3961345`, `afebcb8` | The release sequence is now an executable stop-the-world/drain sequence (stop API + AdminJS, apply `0053` from the sole migration owner with nothing serving, start the new revision, verify), and the zero-downtime alternative is documented as requiring an intermediate release that this artifact does not provide. Corrected consistently in the handoff §6, ADR 0007, the deployment guide §4, and this document. No code or migration changed. |
| P2 — flaky `owner-post-closure` race assertion accepted only one of two legitimate rejections | `e647e03` | The losing approval now accepts either the "already REJECTED" or the closed-post "no longer active" preflight error while retaining the persisted-state assertions (`REJECTED`, not `PENDING`). The unchanged strict assertion in the forced request-vs-closure test is deterministic (the blocked approval is past preflight before closure commits). |

**Re-verification on the merge (`ef14a94`):** backend unit **1026/1026 PASS**; focused integration (`profile-photo-lifecycle`, `comment-image-recovery`, `media-deletion`, `owner-post-closure`, `account-deletion`) **117/117 PASS**. A previous reviewer also independently reran all 41 real integration suites green.

**Known residual (crash-only, pre-existing, documented not fixed):** if a process crashes in the narrow window where account deletion has committed (cascading the ticket away) but a `PutObject` for a profile photo lands afterwards, the object can remain with no ticket to rescan. Adding an `avatars/{userId}/` prefix sweep in `AccountDeletionService.cleanupStorageData` alongside the existing `staging/{userId}/` sweep would only narrow that window, and is **not** a complete solution: a `PutObject` that was already in flight can still complete *after* the sweep has run, recreating the object with no ticket to rescan. A robust fix additionally requires durable finalization coordination for profile photos — an in-flight finalization obligation that account deletion drains before sweeping, mirroring the comment-media finalization obligation — or a later orphan-reconciliation pass that covers objects no ticket references. No non-crash interleaving can delete activation-installed bytes or leave an object with no durable cleanup path.

---

## 17. Third post-review round (PR review of `bb14a69`)

One further P2 finding plus one documentation correction were fixed on `task/bwg-fix4` and merged (`ba68038`); both were independently reviewed with red/green reproduction, and the reviewer added one blocking regression fix during review.

| Finding | Fix commits | Resolution and evidence |
|---|---|---|
| P2 — expired-upload cleanup could delete an active avatar (`cleanupExpiredStaging` checked only `comment_media` before deleting permanent bytes; a stale candidate could terminalize an activated ticket and delete live bytes with no crash required) | `c03e030`, `4ee2a14`, `ced8591`, `36bfac5` | `cleanupExpiredStaging` now routes every permanent-key candidate through the shared coordinated `reclaimCandidate`: `PROFILE_PHOTO` locks the owner row and re-checks `profile_photo_storage_key`; `COMMENT_IMAGE` re-checks `comment_media` in-transaction; `POST_MEDIA` re-checks `post_media.cloudflare_storage_key` and fresh `media_finalizations` in-flight obligations. The conditional terminal transition excludes consumed (`FINALIZED`) tickets, and `finalizeCommentImages`/`finalizeMedia` now discard/queue their published objects and fail retryably when they lose the `CLAIMED → FINALIZED` transition. Residuals closed during review: the no-key `POST_MEDIA` in-flight state is now skipped by obligation (not terminalized), a discarded post-media key is durably latched so the stale-`FAILED` scan can reclaim it, and the latch cannot be clobbered by a slow concurrent cleanup pass (`36bfac5`). Reviewer reproduced the original repro red pre-fix (active bytes deleted) and green post-fix. |
| Documentation — §16 crash residual | `ab66d25` | Corrected: an `avatars/{userId}/` prefix sweep only narrows the crash window; durable finalization coordination (an in-flight obligation for profile photos, mirroring comment media/post media) or a later orphan-reconciliation pass is still required. |

**Re-verification on the merge (`ba68038`):** backend unit **1028/1028 PASS**; focused integration (`legacy-post-upload`, `profile-photo-lifecycle`, `comment-image-recovery`, `media-deletion`, `account-deletion`) **119/119 PASS**. Independent reviewer red/green: pre-fix the avatar repro deleted the active final key and the comment/post in-flight tests committed against reclaimed bytes; post-fix all pass.

**Known residual (pre-existing, non-blocking, documented not fixed):** a hard process crash between `CopyObject` and the `CLAIMED → FINALIZED` update in `finalizeMedia` can still leave a copied `posts/` object with no reference and no reconciler (storage-cost only; no live-bytes risk). A durable post-media orphan-reconciliation pass would close it. Also pre-existing: a successful finalization whose post-copy staging delete fails only logs (staging leak), and concurrent duplicate `finalizeMedia` calls for the same `(mediaId, postId)` are unreachable through the current API but would delete the winner's bytes if they were.
